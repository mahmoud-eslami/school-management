    ...
    def post(self, request):
        try:
            ...
            if userDoc.objects.all().filter(nationalCode=national_code).exists():
                ...
                print(code)
                send_mail('کد دومرحله ای تغییر رمز عبور.', 'Your reset code is {0}'.format(
                    code), settings.EMAIL_HOST_USER, ['{0}'.format(email), ], fail_silently=False)
    ...
    def post(self, request):
        try:
            ...
            if MyUser.objects.all().filter(id=user_id).exists():
                ...
                if role == '1':
                    serializer = serializers.ModeratorChangePasswordSerializer(
                        data=request.data)
...
class ChangePassInProfile(APIView):
    ...
    def post(self, request):
        try:
            ...
            temp_user = MyUser.objects.get(id=user_id)
            serializer = serializers.ChangePasswordInProfileSerilizer(
                data=request.data)
            if serializer.is_valid():
        ...
        try:
            ...
            if role == '4' and role == '3':
                ...
                user_serializer = serializers.UserSerializer(
                    temp_user, data=request.data)
                userDoc_serializer = serializers.UserDocSerializer(
                    temp_userDoc, data=request.data)
                if user_serializer.is_valid():
                    if userDoc_serializer.is_valid():
                        user_serializer.save()
                        userDoc_serializer.save()
                        temp_user.username = userDoc_serializer.data.get(
                            'nationalCode')
                        temp_user.save()
    ...
    def put(self, request):
        try:
            ...
            if userDoc.objects.all().filter(user_id=user_id).exists():
                ...
                return CustomResponse(self, status_code=406, errors=["کاربر با این ایدی وجود ندارد"], message="", data="", status=status.HTTP_406_NOT_ACCEPTABLE)
            serializer = serializers.UserDocSerializer(
                temp_userDoc, request.data)
            if serializer.is_valid():
                serializer.save()
